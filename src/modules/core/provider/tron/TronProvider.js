  ...
  async getTransactions(wallet): Promise<Object> {
    try {
      ...
      for (let i = 0; i < temp.length; i++) {
        const item = { ...temp[i] };
        const triggerInfo = item.trigger_info;
        const isToken = !_.isNil(triggerInfo);
        ...
        item.value = isToken ? TronWeb.fromSun(triggerInfo.parameter._value) : TronWeb.fromSun(item.amount);

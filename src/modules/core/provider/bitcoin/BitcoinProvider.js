...
export class BitcoinProvider implements Provider {
  ...
  async estimateGas(params): Promise<Object> {
    try {
      ...
      if (takerFee && takerAddress) {
        feeAmount = BitcoinUtil.toSatoshi((parseFloat(value) * parseFloat(takerFee)) / 100);
        if (feeAmount < 600) {
          feeAmount = 600;
        }
      }
  ...
  async sendTransaction(transaction): Promise<Object> {
    try {
      ...
      const minimumRequiredAmount = withdrawAmount + BitcoinUtil.toSatoshi(gasFee);
      for (let i = 0; i < utxo.length; i++) {
        totalAmount += utxo[i].value;
  ...
  async getTransactions(wallet): Promise<Object> {
    ...
    for (let i = 0; i < transactions.length; i++) {
      ...
      transactions[i].explore = configProperties.btc.explore + `tx/${transactions[i].txid}`;

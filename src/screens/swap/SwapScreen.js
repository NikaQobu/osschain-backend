...
export default function SwapScreen({ navigation, route }) {
  ...
  const swap = async () => {
    ...
    try {
      ...
      const wallet = await WalletFactory.getWallet(activeWallet.activeAsset.chain);
      ...
      if (fromToken.isNative === false) {
        ...
        if (
          ...
          Logs.info(
            "Approval transaction: " + JSON.stringify(tx),
          );
          await sleep(1500);
        }
      }
      ...
      if (_.isEmpty(exactQuote) || (!_.isNil(exactQuote.code) && exactQuote.code !== 200)) {
        return;
      }
        ...
        <SafeAreaView style={styles.container}>
          <ScrollView
            ...
            <View style={styles.content}>
                ...
                <CommonTouchableOpacity
                  ...
                  {_.isEmpty(fromToken) && (
                    <>
                      ...
                      <CommonText style={{ color: theme.text }}>{t("swap.select")}</CommonText>
                    </>
                  )}
                  {!_.isEmpty(fromToken) && (
                    <>
                      ...
                      />
                      <CoinBalance style={{ color: theme.text }} numberOfLines={1}>
                        {fromToken.balance}
                      </CoinBalance>
                      <CommonText style={{ fontSize: 9, color: theme.text }}>
                        {fromToken.symbol}
                      </CommonText>
                    </>
                  )}
              ...
              <View style={[styles.quoteContainer, { borderBottomColor: theme.border }]}>
                <CommonText style={{ color: theme.text }}>
                  {t("swap.price")} 1 {fromToken.symbol}
                </CommonText>
                {
                  quote && <CoinBalance style={{ color: theme.text }} symbol={toToken.symbol}>
                    {quote.price}
                  </CoinBalance>
                }
              </View>
              <CommonTouchableOpacity
                ...
                <CommonText style={{ color: theme.text }}>{t("swap.slippage")}</CommonText>
                <CommonText style={{ color: theme.text }}>{slippagePercentage * 100}%</CommonText>
              </CommonTouchableOpacity>
              <View style={[styles.quoteContainer, { borderBottomColor: theme.border }]}>
                <CommonText style={{ color: theme.text }}>{t("swap.estimated_gas")}</CommonText>
                {
                  quote && <CoinBalance style={{ color: theme.text }}>
                    {calculateFee(quote.gas, quote.gasPrice)}
                  </CoinBalance>
                }
              </View>
              <View style={[styles.quoteContainer, { borderBottomColor: theme.border }]}>
                <CommonText style={{ color: theme.text }}>{t("swap.allowance")}</CommonText>
                <CommonText style={{ color: theme.text }}>{t("swap.exact_amount")}</CommonText>
              </View>
            </View>

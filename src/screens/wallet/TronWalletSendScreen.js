...
export default function TronWalletSendScreen({ navigation }) {
  ...
  const executeTX = async () => {
    ...
    try {
      ...
      const { success, data } = await WalletFactory.sendTransaction(activeWallet.activeAsset.chain, tx);
      if (success) {
        CommonAlert.show({
          ...
          <ScrollView
            ...
            <ActionSheet
              ...
              <View style={[styles.confirmTx, { backgroundColor: theme.gradientPrimary }]}>
                ...
                <View style={styles.confirmTxItem}>
                  <CommonText style={{ color: theme.text }}>{t("send.bandwidth")}</CommonText>
                  <CommonText
                    style={{ color: theme.text }}>{formatCoins(estimatedGasFee.freeNetUsed)}/{formatCoins(estimatedGasFee.freeNetLimit)}</CommonText>
                </View>
                <View style={styles.confirmTxItem}>
